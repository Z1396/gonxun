...
void MainWindow::updateStatus()
{
    ...
    if (m_markBtn->isChecked()) {
        m_statusLabel->setText(QString("点击红色矩形标记障碍物 | 已标记: %1个").arg(count));
        ...
        m_statusLabel->setText(QString("启动区: %1 (%2) | 障碍物: %3个").arg(zone, pos).arg(count));
        m_statusLabel->setStyleSheet("color: #27ae60; font-weight: bold;");
    } else {
        m_statusLabel->setText(QString("障碍物: %1个").arg(count));
